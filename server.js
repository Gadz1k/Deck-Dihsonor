...
wss.on('connection', ws => {
    // Generujemy unikalne ID dla każdego połączonego klienta
    ...
    ws.on('message', message => {
        ...
        switch (data.type) {
            case 'createGame':
                handleCreateGame(clientId, gameId);
                break;
            case 'joinGame':
                handleJoinGame(clientId, gameId);
                break;
            case 'startGame':
                if (game && game.players.find(p => p.id === clientId)?.isHost) {
                    handleStartGame(gameId);
                }
                break;
            case 'draftCard':
                 if (game && game.currentTurnPlayerId === clientId) {
                    handleDraftCard(clientId, gameId, payload.cardId);
                 }
                break;
            case 'playCard':
                 if (game && game.currentTurnPlayerId === clientId) {
                    handlePlayCard(clientId, gameId, payload.cardId, payload.targetId);
                 }
                break;
        }
    });
    ...
    ws.on('close', () => {
        ...
        if (clientInfo && clientInfo.gameId) {
            ...
            if (game) {
                // Prosta obsługa rozłączenia - usuwamy gracza
...
function handleCreateGame(clientId, gameId) {
    ...
    games[gameId] = {
        ...
        gameState: 'WAITING_FOR_PLAYERS', // WAITING_FOR_PLAYERS, DRAFTING, ACTION, GAME_OVER
...
function handleDraftCard(clientId, gameId, cardId) {
    ...
    // Usuń wszystkie karty z opcji draftu z talii
...
function handlePlayCard(clientId, gameId, cardId, targetId) {
    ...
    // Sprawdź warunki końca tury/gry
...
function applyCardEffect(game, card, casterId, targetId) {
    ...
    switch (card.effect.type) {
        case 'lose_honor':
            if (target) {
                target.honor = Math.max(0, target.honor - card.effect.value);
                game.log.push(`${target.id} traci ${card.effect.value} Honoru.`);
            }
            break;
        case 'lose_honor_and_scandal':
            if (target) {
                target.honor = Math.max(0, target.honor - card.effect.honor);
                target.scandals += card.effect.scandal;
                game.log.push(`${target.id} traci ${card.effect.honor} Honoru i dostaje ${card.effect.scandal} Skandal.`);
            }
            break;
        case 'aoe_lose_honor':
            game.players.forEach(p => {
                if (p.id !== casterId && !p.isEliminated) {
                    p.honor = Math.max(0, p.honor - card.effect.value);
                    game.log.push(`${p.id} traci ${card.effect.value} Honoru.`);
                }
            });
            break;
        case 'gain_honor':
            caster.honor += card.effect.value;
            game.log.push(`${caster.id} zyskuje ${card.effect.value} Honoru.`);
            break;
        case 'extra_turn_and_card':
            if (game.deck.length > 0) {
                caster.hand.push(game.deck.shift());
            }
            game.actionsLeft += 2; // +1 akcja, +1 za dobranie karty
            game.log.push(`${caster.id} dobiera kartę i ma dodatkową akcję.`);
            break;
        case 'leech_honor':
            let honorGained = 0;
            game.players.forEach(p => {
                if (p.id !== casterId && !p.isEliminated && p.honor > 0) {
                    p.honor--;
                    honorGained++;
                }
            });
            caster.honor += honorGained;
            game.log.push(`${caster.id} kradnie ${honorGained} Honoru od innych graczy.`);
            break;
    }
}
...
function endTurn(gameId) {
    ...
    // Sprawdzanie warunków zwycięstwa/przegranej
    game.players.forEach(p => {
        if ((p.honor <= 0 || p.scandals >= 5) && !p.isEliminated) {
            p.isEliminated = true;
            game.log.push(`Gracz ${p.id} odpada z gry!`);
        }
    });
    ...
    if (activePlayers.length <= 1) {
        ...
        game.log.push(winner ? `Wygrywa ${winner.id} jako ostatni na placu boju!` : "Wszyscy odpadli! Brak zwycięzcy.");
    ...
    if (honorWinner) {
        game.log.push(`${honorWinner.id} osiąga 20 Honoru i wygrywa grę!`);
        game.gameState = 'GAME_OVER';
        broadcast(gameId);
        return;
    }
    ...
    if (game.round > 10) {
        const sortedByScandal = [...activePlayers].sort((a, b) => a.scandals - b.scandals);
        const winner = sortedByScandal[0];
        game.log.push(`Koniec rund! Wygrywa ${winner.id} z najmniejszą liczbą Skandali (${winner.scandals})!`);
        game.gameState = 'GAME_OVER';
        broadcast(gameId);
        return;
    }

    // Przygotuj draft dla następnego gracza
    if (game.deck.length < 3) {
        game.deck.push(...shuffle(game.discardPile));
        game.discardPile = [];
        game.log.push("Talia przetasowana z odrzuconych kart.");
    }
